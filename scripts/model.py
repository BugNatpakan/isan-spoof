...
class Model(torch_nn.Module):
    ...
    def forward(self, x, fileinfo=None):
        ...
        if fileinfo is None:
            # Plan B: We don't have filenames, so we just use placeholders
            filenames = ["unknown"] * x.shape[0]
            # Assume full length of the input tensor for each file in batch
            datalength = [x.shape[1]] * x.shape[0]
        else:
            # Plan A: Original code
