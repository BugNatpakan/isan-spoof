...
SOURCE_WAV_DIRS = [
    r"D:\work\AI frontier\project AntiSpoof\isan-spoof\data\source\ai_for_thai\fake\Test",
    r"D:\work\AI frontier\project AntiSpoof\isan-spoof\data\source\ai_for_thai\fake\Train",
]

# 3. 🟢 NEW: Point directly to the TSV files instead of the built txt files
ORIGINAL_TSV_FILES = [
    r"D:\work\AI frontier\project AntiSpoof\isan-spoof\data\source\ai_for_thai\fake\test.tsv",
    r"D:\work\AI frontier\project AntiSpoof\isan-spoof\data\source\ai_for_thai\fake\train.tsv"
]

# 4. Target Destinations (Where the new FLAC files will go)
root_output_dir = r"D:\work\AI frontier\project AntiSpoof\isan-spoof\data\source\ai_for_thai\fake\combine"
    ...
    for source_dir in SOURCE_WAV_DIRS: 
        ...
        for filename in os.listdir(source_dir):
            if filename.endswith('.wav'): 
                file_id = filename.replace('.wav', '').replace('train_', '').replace('test_', '').replace('_', f'_{DEFAULT_LABEL}_').strip()
                file_name = filename.replace('train_', '').replace('test_', '').strip()
    ...
    for tsv_path in ORIGINAL_TSV_FILES:
        ...
        with open(tsv_path, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                try:
                    speaker_id = f"thai_spoof_{int(raw_speaker_id):03d}" 
                except ValueError:
                    speaker_id = f"thai_spoof_{raw_speaker_id}"
