...
SOURCE_WAV_DIRS = [
    r"D:\work\AI frontier\project AntiSpoof\isan-spoof\data\source\ai_for_thai\fake\wav_eval",
    r"D:\work\AI frontier\project AntiSpoof\isan-spoof\data\source\ai_for_thai\fake\wav_trn",
]

# 3. Original Metadata Files (To know which file is spoof/bonafide)
ORIGINAL_METADATA_FILES = [
    r"D:\work\AI frontier\project AntiSpoof\isan-spoof\data\source\ai_for_thai\fake\thaispoof_metadata.eval.txt",
    r"D:\work\AI frontier\project AntiSpoof\isan-spoof\data\source\ai_for_thai\fake\thaispoof_metadata.trn.txt"
]
...
def main():
    ...
    for meta_path in ORIGINAL_METADATA_FILES:
        ...
        with open(meta_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                speaker_id = int(parts[0][1:]) # Column 1 is the Speaker ID 
    ...
    for item in tqdm(final_selection, desc="Converting & Saving"):
        ...
        final_meta_lines.append("TTS_" + str(item["speaker_id"]).zfill(5) + " " + item["file_id"] + " - " + item["label"] + " - \n") # <-- Adjusted to match ASVspoof format
